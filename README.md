Hi, I'm Jessy 👋

🎓 Computer Science Student | University of Ottawa
💻 Interested in Software Development, Web Development, Databases, UI/UX, and Cybersecurity

I'm a Computer Science student at the University of Ottawa with hands-on experience building projects in Java, Python, C#, JavaScript, SQL, and web technologies.

I enjoy turning ideas into practical applications and learning how different parts of a system work together — from database design and backend development to user interfaces.

🛠️ Technical Skills

Languages:
Java • Python • C++ • C# • JavaScript • Go • OCaml • Prolog • SQL

Web & Backend:
HTML • CSS • JavaScript • ASP.NET Core • Entity Framework Core • Spring Boot

Databases:
MySQL • Firebase • SQL • Relational Database Design • Database Normalization

Tools:
Git • GitHub • Android Studio • MySQL Workbench • Linux • NASM

🚀 Featured Projects
🏨 E-Hotels — Hotel Management System

ASP.NET Core • C# • Entity Framework Core • MySQL

Hotel management and reservation system developed as a university project. My main contribution was designing the relational database, including hotels, rooms, customers, employees, bookings, renting, amenities, and other system entities.
🔗 Repository: https://github.com/Jessy-Helaleh/E-Hotels-Web-Application

📱 Event Search Android App

Java • Android Studio • Firebase

Android application for searching and managing events, with Firebase used to store and manage application data.
🔗 Repository: https://github.com/Jessy-Helaleh/Event-Management-Android-App

💇 Hair Salon Website

HTML • CSS • JavaScript

Front-end website designed for a hair salon with a focus on usability, organization, and user-friendly navigation.

🛒 Amazon Survey

UI/UX • User Research

University project focused on analyzing the user experience of an e-commerce platform through user research and survey-based analysis.
🔗 Repository: https://github.com/N3TTI3/SEG3125-group3-lab6

✈️ Explore Trip 

UI/UX • Web Design

Travel-planning interface inspired by platforms such as Expedia, designed to help users explore and organize travel options through an intuitive interface.
🔗 Repository: https://github.com/N3TTI3/SEG3125---Lab-8

🚌 Bus Stop Detection & Counting

A computer vision project built with Python and OpenCV to analyze bus-stop footage, detect activity within a defined region of interest (ROI), and count people in the area.

🔗 Repository: https://github.com/Jessy-Helaleh/Bus_stop

🔐 Cybersecurity

Currently developing my cybersecurity skills through hands-on labs and practical challenges on TryHackMe.

🌐 https://tryhackme.com/p/jhela051
🔎 Practicing networking, Linux, web security, reconnaissance, and cybersecurity fundamentals
🧪 Hands-on experience through interactive security labs and CTF-style challenges

📚 Currently Learning
Software Engineering
Database Systems
Computer Systems
Networking
Cybersecurity
Algorithms & Data Structures
🌐 Connect With Me

📧 Email: jessy.hlaleh@hotmail.com
💼 LinkedIn: Jessy Helaleh


⭐ Feel free to explore my repositories and projects!
